...
class MFN_Options_sliderbar extends Mfn_Options_field
{
	...
	public function render( $meta = false, $vb = false, $js = false )
	{
		...
		$unit = isset( $this->field['param']['unit'] ) ? $this->field['param']['unit'] : 'px';
		...
		if(isset($this->field['units'])){
			...
			if( !empty($this->value) ){
				$clean_value = str_replace(array('px', 'rem', 'em', '%', 'vw', 'vh'), '', $this->value);

				if( strpos($this->value, 'rem') !== false ){
					$unit = 'rem';
				}else if( strpos($this->value, 'em') !== false ){
					$unit = 'em';
				}else if( strpos($this->value, 'vw') !== false ){
					$unit = 'vw';
				}else if( strpos($this->value, 'vh') !== false ){
					$unit = 'vh';
			...
			$clean_value = str_replace( $unit, '', $this->value );
		...
		echo '<div class="'. esc_attr( $class ) .'">';
			...
			if( $meta ){
				if( $js ){
					...
					echo '<input type="hidden" class="mfn-field-value" '. $this->get_name( $meta ) .' value="'. esc_attr( $this->value ) .'">';
				}
			}
			...
			echo '<div class="form-control">';
				if( $js ){
					if( $js_ex_0 ){
						echo '<input class="mfn-form-control mfn-form-input mfn-sliderbar-value" type="number" data-step="'. esc_attr( $step ) .'" data-unit="'. esc_attr( $unit ) .'" min="'. esc_attr( $min ) .'" max="'. esc_attr( $max ) .'" '. ( ! $meta ? $this->get_name( $meta ) : null ) .' value="\'+( '.$js_ex_0.' && '.$js.' && typeof '.$js_ex_0.' !== \'undefined\' && typeof '.$js.' !== \'undefined\' ? '.$js.'.replace(/[a-z]/g, "") : "")+\'" placeholder="'. esc_attr( $placeholder ) .'" />';
					}else{
						echo '<input class="mfn-form-control mfn-form-input mfn-sliderbar-value" type="number" data-step="'. esc_attr( $step ) .'" data-unit="'. esc_attr( $unit ) .'" min="'. esc_attr( $min ) .'" max="'. esc_attr( $max ) .'" '. ( ! $meta ? $this->get_name( $meta ) : null ) .' value="\'+( '.$js.' && typeof '.$js.' !== \'undefined\' ? '.$js.'.replace(/[a-z]/g, "") : "")+\'" placeholder="'. esc_attr( $placeholder ) .'" />';
					}
				}else{
					echo '<input class="mfn-form-control mfn-form-input mfn-sliderbar-value" type="number" data-step="'. esc_attr( $step ) .'" data-unit="'. esc_attr( $unit ) .'" min="'. esc_attr( $min ) .'" max="'. esc_attr( $max ) .'" '. ( ! $meta ? $this->get_name( $meta ) : null ) .' value="'. esc_attr( $clean_value ) .'" placeholder="'. esc_attr( $placeholder ) .'" />';
				}
