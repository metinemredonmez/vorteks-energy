...
if (! function_exists('mfn_ID')) {
	function mfn_ID( $param = false )
	{
		...
		if( !empty($_GET['mfn-template-id']) && is_numeric( $_GET['mfn-template-id'] ) && get_post_status( $_GET['mfn-template-id'] ) == 'publish' && get_post_type( $_GET['mfn-template-id'] ) == 'template' ){
...
if (! function_exists('mfn_header_ID')) {
	function mfn_header_ID()
	{

		$header_tmp_id = get_post_meta( mfn_ID(), 'mfn_header_template', true );

		if( isset( $header_tmp_id ) && is_numeric( $header_tmp_id ) && get_post_status( $header_tmp_id ) == 'publish' && get_post_type( $header_tmp_id ) == 'template' ){
			return $header_tmp_id;
		}
