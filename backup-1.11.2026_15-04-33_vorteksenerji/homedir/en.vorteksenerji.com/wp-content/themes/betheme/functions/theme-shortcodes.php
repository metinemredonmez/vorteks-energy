...
if (! function_exists('sc_header_menu')) {
	function sc_header_menu($attr){
		...
		if( isset($attr['menu-pointer-position']) ){
			$classes .= $attr['menu-pointer-position'];
		}

		$arg = array(
			'container_class' => 'header_column_wrapper '.$classes
		);
...
if (! function_exists('sc_header_icon')) {
	function sc_header_icon($attr){
		...
		$classes = '';
		$attr_link = '#';
		$icon_html = '<span class="mfn-header-icon">';
		$icon_desc = '';

		// icon
		if( !empty($image) ){
			$icon_html .= '<img width="20" src="'.mfn_vc_image($image).'" alt="">';
		}else if( !empty($icon) ){
			$icon_html .= '<i class="'.$icon.'" aria-hidden="true"></i>';
		}else if($type == 'cart' && function_exists('is_woocommerce') && empty($image) && empty($icon)){
			$icon_html .= '<svg viewBox="0 0 26 26" aria-hidden="true"><defs><style>.path{fill:none;stroke:#333;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><polygon class="path" points="20.4 20.4 5.6 20.4 6.83 10.53 19.17 10.53 20.4 20.4"/><path class="path" d="M9.3,10.53V9.3a3.7,3.7,0,1,1,7.4,0v1.23"/></svg>';
		}else if($type == 'account' && function_exists('is_woocommerce') && empty($image) && empty($icon)){
			$icon_html .= '<svg viewBox="0 0 26 26" aria-hidden="true"><defs><style>.path{fill:none;stroke:#333333;stroke-width:1.5px;}</style></defs><circle class="path" cx="13" cy="9.7" r="4.1"/><path class="path" d="M19.51,18.1v2.31h-13V18.1c0-2.37,2.92-4.3,6.51-4.3S19.51,15.73,19.51,18.1Z"/></svg>';
		}else if($type == 'wishlist' && function_exists('is_woocommerce') && empty($image) && empty($icon)){
			$icon_html .= '<svg viewBox="0 0 26 26" aria-hidden="true"><defs><style>.path{fill:none;stroke:#333;stroke-width:1.5px;}</style></defs><path class="path" d="M16.7,6a3.78,3.78,0,0,0-2.3.8A5.26,5.26,0,0,0,13,8.5a5,5,0,0,0-1.4-1.6A3.52,3.52,0,0,0,9.3,6a4.33,4.33,0,0,0-4.2,4.6c0,2.8,2.3,4.7,5.7,7.7.6.5,1.2,1.1,1.9,1.7H13a.37.37,0,0,0,.3-.1c.7-.6,1.3-1.2,1.9-1.7,3.4-2.9,5.7-4.8,5.7-7.7A4.3,4.3,0,0,0,16.7,6Z"/></svg>';
		}else if($type == 'search' && empty($image) && empty($icon)){
			$icon_html .= '<svg width="26" viewBox="0 0 26 26" aria-hidden="true"><defs><style>.path{fill:none;stroke:#000;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><circle class="path" cx="11.35" cy="11.35" r="6"/><line class="path" x1="15.59" y1="15.59" x2="20.65" y2="20.65"/></svg>';
		}

		$icon_html .= '</span>';

		// additional fo cart
		if($type == 'cart' && function_exists('is_woocommerce') && ( $attr['cart_total'] == 1 || $attr['cart_count'] == 1 ) ){
			global $woocommerce;

			if($attr['cart_count'] == 1) $icon_html .= '<span class="header-cart-count">1</span>';
			if($attr['cart_total'] == 1) $icon_html .= '<p class="header-cart-total">'. wp_strip_all_tags( $woocommerce->cart->get_cart_total() ) .'</p>';
		}

		// additional to wishlist
		if($type == 'wishlist' && function_exists('is_woocommerce') && $attr['wishlist_count'] == 1 ){
			$icon_html .= '<span class="header-wishlist-count">1</span>';
		}

		// additional to my account
		if($type == 'account' && function_exists('is_woocommerce')){
			$icon_desc .= '<div class="mfn-header-login woocommerce '. ( is_user_logged_in() ? "mfn-header-modal-nav" : "mfn-header-modal-login" ) .'">';
			$icon_desc .= '<span class="mfn-close-icon toggle-login-modal close-login-modal"><span class="icon">&#10005;</span></span>';
			if( ! is_user_logged_in()){
				$icon_desc .= '<h4>'.esc_html( 'Login', 'woocommerce' ).'</h4>';
				ob_start();
				woocommerce_login_form();
				$icon_desc .= ob_get_clean();
				$opt_reg = get_option( 'woocommerce_enable_myaccount_registration' );
				if( $opt_reg == 'yes' ){
					$icon_desc .= '<p class="create_account"><a href="'.get_permalink( get_option('woocommerce_myaccount_page_id') ).'">';
		...
		if(in_array($type, array('default', 'mail', 'tel'))){
			if($type == 'tel' && !empty($desc)) $attr_link = 'tel:'.$desc;
			if($type == 'mail' && !empty($desc)) $attr_link = 'mailto:'.$desc;
			if($type == 'default') $attr_link = $link;
			if(!empty($desc)) $icon_desc .= '<span class="mfn-header-icon-desc">'.$desc.'</span>';
			$classes .= ' mfn-header-link';
		}else if( $type == 'account' && function_exists('is_woocommerce') ){
			$attr_link = wc_get_page_permalink( 'myaccount' );
			$classes .= ' mfn-header-account-link toggle-login-modal';
		}else if( $type == 'cart' && function_exists('is_woocommerce') ){
			$attr_link = wc_get_page_permalink( 'cart' );
			$classes .= ' mfn-header-cart-link toggle-mfn-cart';
		}else if( $type == 'wishlist' && function_exists('is_woocommerce') ){
			$attr_link = get_permalink(mfn_opts_get('shop-wishlist-page'));
			$classes .= ' mfn-header-wishlist-link';
		}else if( $type == 'search' ){
			$classes .= ' mfn-header-search-link mfn-search-button';
		}else if( $type == 'menu' ){
			$classes .= ' mfn-tmpl-burger';
		}

		$output .= '<a '. ( $type == 'default' && isset($attr['link_target']) && $attr['link_target'] == '1' ? 'target="_blank"' : null ) .' href="'.$attr_link.'" class="header_column_wrapper '.$classes.'">';
			$output .= $icon_html;
			$output .= $icon_desc;
		$output .= '</a>';
		return $output;
...
if (! function_exists('sc_header_promo_bar')) {
	function sc_header_promo_bar($attr){
		...
		$output = '<div class="promo_bar_slider">';

		if ( is_array( $mfn_tabs_array ) ) {
			foreach ( $mfn_tabs_array as $tab ) {
				$output .= '<div class="pbs_one">';
				if(!empty($tab['title'])) $output .= '<strong>'.$tab['title'].' </strong> ';
...
if (! function_exists('sc_product_content')) {
	function sc_product_content($attr, $product = false)
	{
		...
		if( !$product ){
			/*$sample = Mfn_Builder_Woo_Helper::sample_item('product');
			$product = wc_get_product($sample);*/
			$output = '<div class="mfn-widget-placeholder mfn-wp-product-content"><img class="item-preview-image" src="'.get_theme_file_uri('/muffin-options/svg/placeholders/builder-content.svg').'" alt="product rating"></div>';
...
if (! function_exists('sc_shop_products')) {
	function sc_shop_products($attr, $sample = false){
		...
		if(function_exists('is_woocommerce') && is_woocommerce() && have_posts()):
					...
					$output .= '<div class="mcb-column-inner">';
						...
						$output .= '<div class="filters_wrapper">';
							...
							$output .= '<ul class="categories">';
								...
								$output .= '<li class="close"><a href="#"><i class="icon-cancel" aria-label="icon close"></i></a></li>';
					...
					$output .= '<li class="'. esc_attr(implode(' ', get_post_class())) .'">';
						...
						$output .= '<div class="desc">';
							...
							$output .= '<div class="desc_footer">';
								$output .= '<span class="date"><i class="icon-clock" aria-label="published"></i> '. esc_html(get_the_date()) .'</span>';
								if (comments_open()) {
									$output .= '<i class="icon-comment-empty-fa" aria-label="comments number"></i> <a href="'. esc_url(get_comments_link()) .'" class="post-comments">'. intval(get_comments_number(), 10) .'</a>';
								}
									...
									$output .= '<div class="post-meta clearfix">';
										...
										if (comments_open() && mfn_opts_get('blog-comments')) {
											$output .= '<span class="comments">';
												$output .= '<i class="icon-comment-empty-fa" aria-label="comments number"></i> <a href="'. esc_url(get_comments_link()) .'" class="post-comments">'. esc_html(get_comments_number()) .'</a>';
											$output .= '</span>';
										}
					...
					$output .= '<ul>';
						...
						if ($address) {
							$output .= '<li class="address">';
								$output .= '<span class="icon"><i class="icon-location" aria-label="address"></i></span>';
						...
						if ($telephone) {
							$output .= '<li class="phone phone-1">';
								$output .= '<span class="icon"><i class="icon-phone" aria-label="phone"></i></span>';
						...
						if ($fax) {
							$output .= '<li class="phone fax">';
								$output .= '<span class="icon"><i class="icon-print" aria-label="fax"></i></span>';
						...
						if ($email) {
							$output .= '<li class="mail">';
								$output .= '<span class="icon"><i class="icon-mail" aria-label="mail"></i></span>';
			...
			if ($link || $link_image) {
				...
				$output .= '<div class="image_frame element_classes image_item scale-with-grid'. esc_attr($class_div) .'" '. $style_escaped .' role="link" tabindex="0">';
					...
					$output .= '<div class="image_wrapper">';
						...
						// This variable has been safely escaped above in this function
						$output .= '<a href="'. esc_url($link_all) .'" '. $rel_escaped .' '. $target_escaped .' tabindex="-1">';
							$output .= '<div class="mask"></div>';
							// This variable has been safely escaped above in this function
						...
						$output .= '<div class="image_links '. esc_attr($double_link) .'">';
							if ($link_image) {
								$output .= '<a href="'. esc_url($link_image) .'" class="zoom" rel="prettyphoto" tabindex="-1"><svg viewBox="0 0 26 26" aria-label="zoom image"><defs><style>.path{fill:none;stroke:#333;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><circle cx="11.35" cy="11.35" r="6" class="path"/><line x1="15.59" y1="15.59" x2="20.65" y2="20.65" class="path"/></svg></a>';
							}
							if ($link) {
								// This variable has been safely escaped above in this function
								$output .= '<a href="'. esc_url($link) .'" class="link" '. $target_escaped .' tabindex="-1"><svg viewBox="0 0 26 26" aria-label="go to link"><defs><style>.path{fill:none;stroke:#333;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><g><path d="M10.17,8.76l2.12-2.12a5,5,0,0,1,7.07,0h0a5,5,0,0,1,0,7.07l-2.12,2.12" class="path"></path><path d="M15.83,17.24l-2.12,2.12a5,5,0,0,1-7.07,0h0a5,5,0,0,1,0-7.07l2.12-2.12" class="path"></path><line x1="10.17" y1="15.83" x2="15.83" y2="10.17" class="path"></line></g></svg></a>';
							}
						$output .= '</div>';
...
if (! function_exists('sc_icon_bar')) {
	function sc_icon_bar($attr, $content = null)
	{
		...
		$output = '<a href="'. esc_url($link) .'" class="icon_bar '. esc_attr($class) .'" '. $target_escaped .'>';
			$output .= '<span class="t"><i class="'. esc_attr($icon) .'" aria-label="button with icon"></i></span>';
...
if (! function_exists('sc_alert')) {
	function sc_alert($attr, $content = null)
	{
		...
		switch ($style) {
			case 'error':
				$icon = '<svg viewBox="0 0 28 28" aria-label="error alert icon"><defs><style>.path{fill:none;stroke:#000;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><g><circle cx="14" cy="20" r="0.33" class="path"/><line x1="14" y1="8.72" x2="14" y2="16.72" class="path"/><path d="M12.6,3.42,1.54,22.58A1.61,1.61,0,0,0,2.93,25H25.07a1.61,1.61,0,0,0,1.39-2.42L15.4,3.42A1.61,1.61,0,0,0,12.6,3.42Z" class="path"/></g></svg>';
				break;
			case 'info':
				$icon = '<svg viewBox="0 0 28 28" aria-label="info alert icon"><defs><style>.path{fill:none;stroke:#000;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><g><circle class="path" cx="14" cy="14" r="12"/><path class="path" d="M11.2,9.12a3.4,3.4,0,0,1,3-1.69,2.84,2.84,0,0,1,3,2.76,3.16,3.16,0,0,1-.84,2.23c-.63.74-1.58,1.18-2.19,1.88a1,1,0,0,0-.26.64v2.32"/><circle class="path" cx="14" cy="20" r="0.33"/></g></svg>';
				break;
			case 'success':
				$icon = '<svg viewBox="0 0 28 28" aria-label="success alert icon"><defs><style>.path{fill:none;stroke:#000;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><g><polyline points="8.07 13 12.36 18.29 19.93 9.71" class="path"/><circle cx="14" cy="14" r="12" class="path"/></g></svg>';
				break;
			default:
				$icon = '<svg viewBox="0 0 28 28" aria-label="warning alert icon"><defs><style>.path{fill:none;stroke:#000;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><g><circle class="path" cx="14" cy="14" r="12"/><line class="path" x1="11.5" y1="17.25" x2="16.5" y2="17.25"/><path class="path" d="M16.46,17.8A3.94,3.94,0,0,1,17.64,15a4.61,4.61,0,0,0-.12-6.65A5.09,5.09,0,0,0,14,7h0A5.14,5.14,0,0,0,10.5,8.36,4.66,4.66,0,0,0,9,11.63a4.6,4.6,0,0,0,1.4,3.43,3.85,3.85,0,0,1,1.14,2.74h0V19c0,.87.59,2,1.67,2h1.58c1.08,0,1.67-1.16,1.67-2Z"/></g></svg>';
				break;
		}
		...
		$output = '<div class="blockquote">';
			...
			if ($author) {
				$output .= '<p class="author">';
					...
					$output .= !empty($icon_author) ? '<i class="'.$icon_author.'"></i>' : '<i class="icon-user" aria-label="author"></i>';
	...
	function sc_our_team($attr, $content = null)
	{
		...
		$output = '<div class="team team_'. esc_attr($style) .'">';
				...
				$output .= '<div class="desc_wrapper clearfix">';
					...
					if ($phone) {
						$output .= '<p class="phone"><i class="icon-phone" aria-label="phone icon"></i> <a href="tel:'. esc_attr($phone) .'">'. esc_html($phone) .'</a></p>';
					}
					$output .= '<hr class="hr_color" />';
					...
					if ($email || $phone || $facebook || $twitter || $linkedin) {
						$output .= '<div class="links">';
							if ($email) {
								$output .= '<a href="mailto:'. esc_attr($email) .'" class="icon_bar icon_bar_small mail"><span class="t"><i class="icon-mail" aria-label="mail icon"></i></span><span class="b"><i class="icon-mail" aria-hidden="true"></i></span></a>';
							}
							if ($facebook) {
								$output .= '<a target="_blank" href="'. esc_url($facebook) .'" class="icon_bar icon_bar_small facebook"><span class="t"><i class="icon-facebook" aria-label="facebook icon"></i></span><span class="b"><i class="icon-facebook" aria-hidden="true"></i></span></a>';
							}
							if ($twitter) {
								$output .= '<a target="_blank" href="'. esc_url($twitter) .'" class="icon_bar icon_bar_small twitter"><span class="t"><i class="icon-twitter" aria-label="twitter icon"></i></span><span class="b"><i class="icon-twitter" aria-hidden="true"></i></span></a>';
							}
							if ($linkedin) {
								$output .= '<a target="_blank" href="'. esc_url($linkedin) .'" class="icon_bar icon_bar_small linkedin"><span class="t"><i class="icon-linkedin" aria-label="linkedin icon"></i></span><span class="b"><i class="icon-linkedin" aria-hidden="true"></i></span></a>';
							}
							if ($vcard) {
								$output .= '<a href="'. esc_url($vcard) .'" class="icon_bar icon_bar_small vcard"><span class="t"><i class="icon-vcard" aria-label="vcard icon"></i></span><span class="b"><i class="icon-vcard" aria-hidden="true"></i></span></a>';
							}
						$output .= '</div>';
					}
...
if (! function_exists('sc_map')) {
	function sc_map($attr, $content = null)
	{
		...
		if( !isset($attr['vb']) ) wp_add_inline_script('google-maps', $inline_script);
					...
					$output .= '<div class="get_in_touch">';
						...
						$output .= '<div class="get_in_touch_wrapper">';
							$output .= '<ul>';
								if ($content) {
									$output .= '<li class="address">';
										$output .= '<span class="icon"><i class="icon-location" aria-label="address icon"></i></span>';
										$output .= '<span class="address_wrapper">'. do_shortcode($content) .'</span>';
									$output .= '</li>';
								}
								if ($telephone) {
									$output .= '<li class="phone">';
										$output .= '<span class="icon"><i class="icon-phone" aria-label="phone icon"></i></span>';
										$output .= '<p><a href="tel:'. esc_attr(str_replace(' ', '', $telephone)) .'">'. esc_html($telephone) .'</a></p>';
									$output .= '</li>';
								}
								if ($email) {
									$output .= '<li class="mail">';
										$output .= '<span class="icon"><i class="icon-mail" aria-label="mail icon"></i></span>';
										$output .= '<p><a href="mailto:'. esc_attr($email) .'">'. esc_html($email) .'</a></p>';
									$output .= '</li>';
								}
								if ($www) {
									$output .= '<li class="www">';
										$output .= '<span class="icon"><svg viewBox="0 0 26 26" aria-label="website icon"><defs><style>.path{fill:none;stroke:#333;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><g><path d="M10.17,8.76l2.12-2.12a5,5,0,0,1,7.07,0h0a5,5,0,0,1,0,7.07l-2.12,2.12" class="path"></path><path d="M15.83,17.24l-2.12,2.12a5,5,0,0,1-7.07,0h0a5,5,0,0,1,0-7.07l2.12-2.12" class="path"></path><line x1="10.17" y1="15.83" x2="15.83" y2="10.17" class="path"></line></g></svg></span>';
		...
		$output = '<div class="progress_icons" data-active="'. esc_attr($active) .'" data-color="'. esc_attr($background) .'">';
			for ($i = 1; $i <= $count; $i++) {
				if ($image) {
					...
					$output .= '<span class="progress_icon"><i class="'. esc_attr($icon) .'" aria-label="progress icon"></i></span>';
				}
			}
		$output .= '</div>'."\n";
...
if (! function_exists('sc_table_of_contents')) {
	function sc_table_of_contents($attr, $content = null)
	{
		...
		$output = '<div class="table_of_content" data-tags="'.$tags_anchors.'">';
			...
			if ($title) {
				$output .= '<'.strtolower($title_tag).' class="title">'. wp_kses($title, mfn_allowed_html()) .'</'.strtolower($title_tag).'>';
			}
