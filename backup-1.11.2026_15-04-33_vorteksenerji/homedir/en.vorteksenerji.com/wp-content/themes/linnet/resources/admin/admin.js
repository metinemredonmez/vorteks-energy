...
// admin notice
(function ($) {
    var $notice_container = $('.linnet-admin-big-notice--container');
    ...
    function disableNotice() {
        wp.ajax.post('linnet_disable_big_notice', {
            nonce: linnet_builder_status.linnet_disable_big_notice_nonce
        });
    }

    function toggleProcessing(value) {
        $(window).on('beforeunload.linnet-admin-big-notice', function () {
            return true;
        });
        if (value) {
            $('.linnet-admin-big-notice').addClass('processing');
            $('.linnet-admin-big-notice .action-buttons').fadeOut();
        } else {
            $('.linnet-admin-big-notice').removeClass('processing');
        }
    }
    ...
    function installBuilder(callback) {
        pluginNotice(linnet_builder_status.messages.installing);
        $.get(linnet_builder_status.install_url).always(function () {
            toggleProcessing(true);
            activateBuilder(callback);
        }).always(function () {
            $(window).off('beforeunload.linnet-admin-big-notice');
        });
    }

    function activateBuilder(callback) {
        pluginNotice(linnet_builder_status.messages.activating);

        wp.ajax.post('linnet_activate_plugin', {
            slug: linnet_builder_status.slug,
            source: 'notice-' + selectedFrontPage,
            _wpnonce: linnet_builder_status.colibri_plugin_install_activate_nonce
        }).done(function () {
            $(window).off('beforeunload.linnet-admin-big-notice');
            if (callback) {
                callback();
    ...
    function processBuilderInstalationStepts(callback) {
        wp.ajax.post('linnet_front_set_predesign', {
            index: selectedFrontPage,
            nonce: linnet_builder_status.linnet_front_set_predesign_nonce
        });
        if (linnet_builder_status.status === "not-installed") {
            toggleProcessing(true);
            installBuilder(callback);
        }

        if (linnet_builder_status.status === "installed") {
            toggleProcessing(true);
            activateBuilder(callback);
        }
    }
    ...
    $notice_container.on('click', '.view-all-demos', function () {
        ...
        processBuilderInstalationStepts(function () {
            window.location = linnet_builder_status.view_demos_url;
        });
    });
    ...
    var linnetInstallPluginSuccess = function linnetInstallPluginSuccess(response) {
    ...
    var linnetInstallPlugin = function linnetInstallPlugin(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: linnetInstallPluginSuccess
        });
    };

    var linnetActivatePlugin = function linnetActivatePlugin(event) {
        var $button = $(event.target);
    ...
    $document.on("click", ".colibri-plugin.install-now", linnetInstallPlugin);
    $document.on("click", ".colibri-plugin.activate-now", linnetActivatePlugin);
