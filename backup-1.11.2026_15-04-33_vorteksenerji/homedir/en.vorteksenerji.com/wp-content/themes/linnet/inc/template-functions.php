<?php
	...
	function linnet_page_title( $atts = array() ) {
		if ( is_404() ) {
			$title = __( 'Page not found', 'linnet' );
		} elseif ( is_search() ) {
			$title = sprintf( __( 'Search Results for &#8220;%s&#8221;', 'linnet' ), get_search_query() );
		} elseif ( is_home() ) {
			if ( is_front_page() ) {
	...
	function linnet_site_title() {
		$site_title = get_bloginfo( 'name' );
		echo esc_html( $site_title );
	}

	function linnet_post_title( $atts ) {
		$atts = array_merge(
			array(
				'heading_type' => 'h3',
				'classes'      => 'linnet-word-wrap'
			),
			$atts
		);
	...
	function linnet_post_excerpt( $attrs = array() ) {

		echo '<div class="colibri-post-excerpt">' . get_the_excerpt() . '</div>';
	}

	function linnet_post_thumb_placeholder_classes( $atts = array() ) {
	...
	function linnet_post_thumbnail_classes( $atts = array() ) {
	...
	function linnet_post_thumbnail( $atts = array() ) {
	...
	function linnet_post_meta_date_url( $atts = array() ) {
		$id   = get_the_ID();
	...
	function linnet_post_categories( $attrs = array() ) {
		...
		$html = "";
		if ( $atts['prefix'] !== '' ) {
			$html .= '<span class="d-inline-block categories-prefix">' . linnet_esc_html_preserve_spaces( $atts['prefix'] ) . '</span>';
		}
		if ( $categories ) {
			...
			$html .= sprintf( '<span class="d-inline-block">%s</span>', esc_html__( 'No Category', 'linnet' ) );
		}

		echo $html;
	}

	function linnet_esc_html_preserve_spaces( $text ) {
		return esc_html( str_replace( " ", "&nbsp;", $text ) );
	}

	function linnet_post_tags( $attrs = array() ) {
		$atts = shortcode_atts(
			array(
				'prefix' => '',
			),
		...
		$html = '';
		if ( $atts['prefix'] !== '' ) {
			$html .= '<span class="d-inline-block tags-prefix">' . linnet_esc_html_preserve_spaces( $atts['prefix'] ) . '</span>';
		}
		if ( $tags ) {
			foreach ( $tags as $tag ) {
				$tag_link  = get_tag_link( $tag->term_id );
				$tag_title = sprintf( __( 'Tag: %s', 'linnet' ), $tag->name );
			...
			$html .= sprintf( '<span class="d-inline-block">%s</span>', esc_html__( 'No Tag', 'linnet' ) );
		}
	...
	function linnet_get_nav_direction_wp_name( $type ) {
		return $type == "next" ? $type : "previous";
	}


	function linnet_print_navigation_button( $type, $button_text ) {
		$args = array(
			'prev_text'          => '%title',
			'next_text'          => '%title',
			'in_same_term'       => false,
			'excluded_terms'     => '',
			'taxonomy'           => 'category',
			'screen_reader_text' => __( 'Post navigation', 'linnet' ),
		);

		$navigation        = '';
		$direction_wp_name = linnet_get_nav_direction_wp_name( $type );
	...
	function linnet_post_nav_button( $atts = array() ) {
		$type = $atts['type'];
		...
		linnet_print_navigation_button( $type, $button_text );

	}


	function linnet_button_pagination( $args, $atts ) {
		$type          = $atts['type'];
		$nav_direction = linnet_get_nav_direction_wp_name( $type );
	...
	function linnet_numbers_pagination( $args, $atts ) {
	...
	function linnet_render_pagination( $pagination_type, $atts = array(), $args = array() ) {
	...
	function linnet_archive_nav_button( $attrs = array() ) {
		...
		linnet_render_pagination( 'linnet_button_pagination', $atts );
	}


	function linnet_archive_pagination() {
		linnet_render_pagination( '\linnet_numbers_pagination' );
	}

	function linnet_render_page_comments() {
		if ( ! comments_open() ) {
			return;
		}
		?>
	...
	function linnet_post_comments( $attrs = array() ) {
		...
		add_filter( 'comments_template', 'linnet_post_comments_template' );
		...
		remove_filter( 'comments_template', 'linnet_post_comments_template' );

		echo $content;
	}

	function linnet_post_comment_form() {

	}

	function linnet_widget_area( $atts ) {
	...
	function linnet_post_meta_time_url() {
		return '';
	}

	function linnet_has_multiple_pages() {
		global $wp_query;
	...
	function linnet_output_sidebar_search_form( $form = '' ) {
	...
	function linnet_post_comments_template( $form = '' ) {
		return 'template-parts/blog/comments.php';
	}

	function linnet_theme_print_footer_copyright() {
		?>
        <div class="h-global-transition-all">
            &copy; <?php echo date( 'Y' ); ?> <?php bloginfo( 'blogname' ); ?>.
			<?php printf( __( 'Built using WordPress and %s', 'linnet' ),
				'Linnet Theme'
			); ?> .
        </div>
		<?php
	...
	function linnet_footer_parallax_is_enabled() {
	...
	function linnet_print_offscreen_copyright() {
		echo "&copy; " . date( 'Y' );
	}

	function linnet_the_date( $format = 'F j, Y' ) {
		echo get_the_date( $format );
	}

	add_filter( 'get_search_form', "linnet_output_sidebar_search_form", 100 );


	function linnet_layout_wrapper( $atts ) {
		$atts = array_merge(
			array(
				"name" => "",
				"slug" => ""
	...
	function linnet_layout_wrapper_output_tags_container( $atts ) {
		if ( has_tag() ) {
	...
	function linnet_layout_wrapper_output_categories_container( $atts ) {
		if ( has_category() ) {
			View::partial( 'layout-wrapper-content', $atts['slug'] );
		}
	}
	...
	function linnet_layout_wrapper_output_navigation_container( $atts ) {
		if ( linnet_has_multiple_pages() ) {
			View::partial( 'layout-wrapper-content', $atts['slug'] );
		}
	}

	Hooks::prefixed_add_action( "layout_wrapper_output_tags_container",
		'linnet_layout_wrapper_output_tags_container' );

	Hooks::prefixed_add_action( "layout_wrapper_output_categories_container",
		'linnet_layout_wrapper_output_categories_container' );

	Hooks::prefixed_add_action( "layout_wrapper_output_navigation_container",
		'linnet_layout_wrapper_output_navigation_container' );
