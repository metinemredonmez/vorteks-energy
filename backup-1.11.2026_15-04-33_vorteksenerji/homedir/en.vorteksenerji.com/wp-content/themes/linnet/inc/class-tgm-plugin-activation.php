 ...
 * @version   2.6.1 for parent theme Linnet for publication on WordPress.org
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        public function init() {
            ...
            // Load class strings.
            $this->strings = array(
                'page_title'                      => __( 'Install Required Plugins', 'linnet' ),
                'menu_title'                      => __( 'Install Plugins', 'linnet' ),
                /* translators: %s: plugin name. */
                'installing'                      => __( 'Installing Plugin: %s', 'linnet' ),
                /* translators: %s: plugin name. */
                'updating'                        => __( 'Updating Plugin: %s', 'linnet' ),
                'oops'                            => __( 'Something went wrong with the plugin API.', 'linnet' ),
                ...
                'return'                          => __( 'Return to Required Plugins Installer', 'linnet' ),
                'dashboard'                       => __( 'Return to the Dashboard', 'linnet' ),
                'plugin_activated'                => __( 'Plugin activated successfully.', 'linnet' ),
                ...
                'complete'                        => __( 'All plugins installed and activated successfully. %1$s',
                    'linnet' ),
                'dismiss'                         => __( 'Dismiss this notice', 'linnet' ),
        ...
        public function filter_plugin_action_links_update( $actions ) {
            $actions['update'] = sprintf(
                ...
                esc_attr__( 'This plugin needs to be updated to be compatible with your theme.', 'linnet' ),
                esc_html__( 'Update Required', 'linnet' )
            );
                    ...
                    if ( true === $GLOBALS['wp_filesystem']->move( $from_path, $to_path ) ) {
                        ...
                        return new WP_Error( 'rename_failed',
                            esc_html__( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.',
                                'linnet' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.',
                                'linnet' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
                    }
                ...
                if ( ! current_user_can( 'activate_plugins' ) && ! current_user_can( 'install_plugins' ) && ! current_user_can( 'update_plugins' ) ) {
                    ...
                    foreach ( $message as $type => $plugin_group ) {
                        ...
                        $rendered .= sprintf(
                            $line_template,
                            sprintf(
                                translate_nooped_plural( $this->strings[ $type ], $count, 'linnet' ),
                                $imploded,
                                $count
                            )
                        );
        ...
        protected function create_user_action_links_for_notice(
            ...
            if ( current_user_can( 'install_plugins' ) ) {
                if ( $install_count > 0 ) {
                    $action_links['install'] = sprintf(
                        $link_template,
                        translate_nooped_plural( $this->strings['install_link'], $install_count, 'linnet' ),
                ...
                if ( $update_count > 0 ) {
                    $action_links['update'] = sprintf(
                        $link_template,
                        translate_nooped_plural( $this->strings['update_link'], $update_count, 'linnet' ),
                        esc_url( $this->get_tgmpa_status_url( 'update' ) )
                    );
                }
            }
            ...
            if ( current_user_can( 'activate_plugins' ) && $activate_count > 0 ) {
                $action_links['activate'] = sprintf(
                    $link_template,
                    translate_nooped_plural( $this->strings['activate_link'], $activate_count, 'linnet' ),
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function get_plugin_advise_type_text( $required ) {
            if ( true === $required ) {
                return __( 'Required', 'linnet' );
            }

            return __( 'Recommended', 'linnet' );
        }
        ...
        protected function get_plugin_source_type_text( $type ) {
            ...
            switch ( $type ) {
                case 'repo':
                    $string = __( 'WordPress Repository', 'linnet' );
                    break;
                case 'external':
                    $string = __( 'External Source', 'linnet' );
                    break;
                case 'bundled':
                    $string = __( 'Pre-Packaged', 'linnet' );
                    break;
            }
        ...
        protected function get_plugin_status_text( $slug ) {
            if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
                return __( 'Not Installed', 'linnet' );
            }

            if ( ! $this->tgmpa->is_plugin_active( $slug ) ) {
                $install_status = __( 'Installed But Not Activated', 'linnet' );
            } else {
                $install_status = __( 'Active', 'linnet' );
            }

            $update_status = '';

            if ( $this->tgmpa->does_plugin_require_update( $slug ) && false === $this->tgmpa->does_plugin_have_update( $slug ) ) {
                $update_status = __( 'Required Update not Available', 'linnet' );

            } elseif ( $this->tgmpa->does_plugin_require_update( $slug ) ) {
                $update_status = __( 'Requires Update', 'linnet' );

            } elseif ( false !== $this->tgmpa->does_plugin_have_update( $slug ) ) {
                $update_status = __( 'Update recommended', 'linnet' );
            }
            ...
            /* translators: 1: install status, 2: update status */
                _x( '%1$s, %2$s', 'Install/Update Status', 'linnet' ),
                $install_status,
                $update_status
            );
        }
                ...
                switch ( $type ) {
                    case 'all':
                        ...
                        $text = _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>',
                            $count, 'plugins', 'linnet' );
                        break;
                    case 'install':
                        /* translators: 1: number of plugins. */
                        $text = _n( 'To Install <span class="count">(%s)</span>',
                            'To Install <span class="count">(%s)</span>', $count, 'linnet' );
                        break;
                    case 'update':
                        /* translators: 1: number of plugins. */
                        $text = _n( 'Update Available <span class="count">(%s)</span>',
                            'Update Available <span class="count">(%s)</span>', $count, 'linnet' );
                        break;
                    case 'activate':
                        /* translators: 1: number of plugins. */
                        $text = _n( 'To Activate <span class="count">(%s)</span>',
                            'To Activate <span class="count">(%s)</span>', $count, 'linnet' );
                        break;
                    default:
                        $text = '';
                        break;
        ...
        public function column_version( $item ) {
            ...
            if ( $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
                $installed = ! empty( $item['installed_version'] ) ? $item['installed_version'] : _x( 'unknown',
                    'as in: "version nr unknown"', 'linnet' );
        ...
        public function no_items() {
            echo esc_html__( 'No plugins to install, update or activate.',
                    'linnet' ) . ' <a href="' . esc_url( self_admin_url() ) . '"> ' . esc_html__( 'Return to the Dashboard',
                    'linnet' ) . '</a>';
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        }
        ...
        public function get_columns() {
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'linnet' ),
                'source' => __( 'Source', 'linnet' ),
                'type'   => __( 'Type', 'linnet' ),
            );

            if ( 'all' === $this->view_context || 'update' === $this->view_context ) {
                $columns['version'] = __( 'Version', 'linnet' );
                $columns['status']  = __( 'Status', 'linnet' );
            }
        ...
        protected function get_row_actions( $item ) {
            ...
            if ( ! $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
                ...
                $actions['install'] = __( 'Install %2$s', 'linnet' );
            } else {
                // Display the 'Update' action link if an update is available and WP complies with plugin minimum.
                if ( false !== $this->tgmpa->does_plugin_have_update( $item['slug'] ) && $this->tgmpa->can_plugin_update( $item['slug'] ) ) {
                    /* translators: %2$s: plugin name in screen reader markup */
                    $actions['update'] = __( 'Update %2$s', 'linnet' );
                }

                // Display the 'Activate' action link, but only if the plugin meets the minimum version.
                if ( $this->tgmpa->can_plugin_activate( $item['slug'] ) ) {
                    /* translators: %2$s: plugin name in screen reader markup */
                    $actions['activate'] = __( 'Activate %2$s', 'linnet' );
                }
            }
        ...
        public function wp_plugin_update_row( $slug, $item ) {
            ...
            esc_html__( 'Upgrade message from the plugin author:', 'linnet' ),
        ...
        public function get_bulk_actions() {
            ...
            if ( 'update' !== $this->view_context && 'activate' !== $this->view_context ) {
                if ( current_user_can( 'install_plugins' ) ) {
                    $actions['tgmpa-bulk-install'] = __( 'Install', 'linnet' );
                }
            }

            if ( 'install' !== $this->view_context ) {
                if ( current_user_can( 'update_plugins' ) ) {
                    $actions['tgmpa-bulk-update'] = __( 'Update', 'linnet' );
                }
                if ( current_user_can( 'activate_plugins' ) ) {
                    $actions['tgmpa-bulk-activate'] = __( 'Activate', 'linnet' );
                }
            }
            ...
            if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
                ...
                if ( empty( $_POST['plugin'] ) ) {
                    if ( 'install' === $install_type ) {
                        $message = __( 'No plugins were selected to be installed. No action taken.', 'linnet' );
                    } else {
                        $message = __( 'No plugins were selected to be updated. No action taken.', 'linnet' );
                    }
                ...
                if ( empty( $plugins_to_install ) ) {
                    if ( 'install' === $install_type ) {
                        $message = __( 'No plugins are available to be installed at this time.', 'linnet' );
                    } else {
                        $message = __( 'No plugins are available to be updated at this time.', 'linnet' );
                    }
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( // WPCS: xss ok.
                        ...
                        esc_html( _n( 'The following plugin was activated successfully:',
                            'The following plugins were activated successfully:', $count, 'linnet' ) ),
                        $imploded
                    );
            ...
            if ( ! class_exists( 'TGMPA_Bulk_Installer' ) ) {
                ...
                class TGMPA_Bulk_Installer extends Plugin_Upgrader {
                    ...
                    public function activate_strings() {
                        $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'linnet' );
                        $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'linnet' );
                    }
                        ...
                        if ( 'update' === $this->options['install_type'] ) {
                            ...
                            if ( $this->tgmpa->is_automatic ) {
                                ...
                                $this->upgrader->strings['skin_update_successful'] = __( '%1$s installed and activated successfully.',
                                        'linnet' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details',
                                        'linnet' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details',
                                        'linnet' ) . '</span>.</a>';
