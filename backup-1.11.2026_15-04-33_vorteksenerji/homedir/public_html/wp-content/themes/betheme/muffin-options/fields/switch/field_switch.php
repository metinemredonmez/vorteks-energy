	...
	public function render($meta = false, $vb = false, $js = false )
	{
		...
		echo '<div class="form-group '. $class .' checkboxes-list">';
			...
			if( !$vb ) echo '<div class="form-control">';
				...
				if( isset($this->field['version']) && $this->field['version'] == 'multiple' ){
					if( $js ){
						echo '<input type="hidden" class="mfn-field-value" value="\'+('.$js.' ? '.$js.' : "")+\'" '. $this->get_name( $meta ) .'>';
					}else{
				...
				echo '<ul class="'. esc_attr( $preview ) .'">';
					...
					foreach ( $this->field['options'] as $k => $v ) {
						...
						if( $js ){
						echo '<li class="\'+('.$js.' && '.$js.' == "'. esc_attr($k) .'" ? "active" : "") +\'" '. $tooltip .' '. $tooltipActive .'>';
						}else{
						echo '<li class="'. $class .'" '. $tooltip .' '. $tooltipActive .'>';
						}
							echo '<fieldset>';
								if( $js ){
									echo '<input type="checkbox" \'+('.$js.' && '.$js.' == "'. esc_attr($k) .'" ? "checked" : "") +\' value="'. esc_attr( $k ) .'" '. $this->get_name( $meta ) .' autocomplete="off" />';
								}else{
