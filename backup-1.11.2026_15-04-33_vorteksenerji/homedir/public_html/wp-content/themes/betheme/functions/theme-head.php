...
if (! function_exists('mfn_body_classes')) {
	function mfn_body_classes($classes)
	{
		...
		if( $header_tmp_id ){
			if( get_post_meta($header_tmp_id, 'header_position', true) != 'default' && !empty( get_post_meta($header_tmp_id, 'body_offset_header', true) ) ){
				$classes[] = 'mfn-body-offset-header';
			}
		}
...
function mfn_visual_builder(){
	...
	if( ! empty( $_GET['visual'] ) && $_GET['visual'] == 'iframe' ){
		...
		function mfnvb_iframe_style() {
			...
			// wp_enqueue_style( 'mfn-luktmp-vbstyle', get_theme_file_uri('/visual-builder/assets/css/luktmp.css'), false, time(), 'all' );
		}
